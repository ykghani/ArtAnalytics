...
class DisplayRatios:
    ...
    def get_suitable_displays(aspect_ratio: float, tolerance: float = 0.1) -> List[str]:
        ...
        return [
            display
            for display, (min_ratio, max_ratio) in RATIO_RANGES.items()
    ...
    def pad_image_for_display(
        ...
        if metadata:
            ...
            try:
                ...
                description_height = draw.textbbox(
                    (0, 0), description, font=font_description
                )[3]
        ...
        if abs(current_ratio - target_aspect_ratio) > 0.1 or metadata_height > 0:
            ...
            if current_ratio > target_aspect_ratio:
                ...
                new_height = (
                    int(original_width / target_aspect_ratio)
                    + additional_vertical_space
                )
                padded = Image.new(
                    "RGB", (original_width, new_height), background_color
                )
                vertical_offset = (
                    new_height - original_height - additional_vertical_space
                ) // 2
                padded.paste(image, (0, vertical_offset))
            else:
                # Image is taller, pad horizontally
                new_width = (
                    int(original_height * target_aspect_ratio)
                    + additional_vertical_space
                )
                padded = Image.new(
                    "RGB", (new_width, original_height), background_color
                )
    ...
    def render_artwork_metadata(
        ...
        description_bbox = draw.textbbox(
            (0, 0), wrapped_description, font=font_description
        )
        ...
        # Render texts
        draw.text((padding, y_offset), wrapped_title, font=font_title, fill=text_color)

        y_offset += title_bbox[3] - title_bbox[1] + padding
        draw.text(
            (padding, y_offset), wrapped_artist, font=font_artist, fill=text_color
        )
