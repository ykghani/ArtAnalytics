...
class ArtworkDownloader:
    ...
    def __init__(
        ...
        self.max_storage_bytes = (
            int(settings.max_storage_gb * 1024 * 1024 * 1024)
            if settings.max_storage_gb
            else None
        )
    ...
    def download_artwork(self, artwork_metadata: ArtworkMetadata) -> None:
        ...
        try:
            ...
            existing_artwork = self.artwork_repo.get_artwork(
                self.client.museum_info.code, artwork_metadata.id
            )
            ...
            if existing_artwork and existing_artwork.image_path:
                ...
                self.progress_tracker.log_status(
                    artwork_metadata.id, "skipped", "Already in database"
                )
                return
