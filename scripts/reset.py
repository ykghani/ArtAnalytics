...
def clean_museum_files(museum_code: Optional[str], logger) -> None:
    ...
    if museum_code:
        ...
        museums_to_clean = {
            code: settings.get_museum_paths(code) for code in settings.museums.keys()
        }
...
def verify_cleanup(museum_code: Optional[str], logger) -> None:
    ...
    with db.get_session() as session:
        if museum_code:
            ...
            if museum:
                artwork_count = (
                    session.query(Artwork).filter_by(museum_id=museum.id).count()
                )
                logger.progress(
                    f"Artwork records remaining for {museum.name}: {artwork_count}"
                )
        else:
...
def verify_directories(museum_code: Optional[str], logger) -> None:
    ...
    for code, paths in museums_to_check.items():
        ...
        if image_path.exists():
            ...
            if image_count > 0:
                logger.error(
                    f"Found {image_count} images in {image_path}, deletion may have failed"
                )
        ...
        if cache_path.exists():
            ...
            if cache_files:
                logger.error(
                    f"Found files in cache directory: {[f.name for f in cache_files]}"
                )
